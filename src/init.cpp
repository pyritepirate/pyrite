...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: pyrite.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: pyrited.pid)") + "\n";
    ...
    strUsage += "  -maxconnections=<n>    " + _("Maintain at most <n> connections to peers (default: 32)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
   ...
   nMaxDatacarrierBytes = GetArg("-datacarriersize", nMaxDatacarrierBytes);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pyrite is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pyrite is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "Pyrite server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pyrite") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Pyrite to complete") << "\n";
