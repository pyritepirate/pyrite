...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            else if (nInbound >= GetArg("-maxconnections", 32) - MAX_OUTBOUND_CONNECTIONS)
            {
                closesocket(hSocket);
            }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Pyrite " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Pyrite is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    if (semOutbound == NULL) {
        ...
        int nMaxOutbound = min(MAX_OUTBOUND_CONNECTIONS, (int)GetArg("-maxconnections", 32));
