...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    while (true)
    {
        ...
        while (vNodes.size() < 5 || IsInitialBlockDownload())
        {
            nLastCoinStakeSearchInterval = 0;
            fTryToSync = true;
            MilliSleep(1000);
        ...
        {
            fTryToSync = false;
            if (vNodes.size() < 5 || pindexBest->GetBlockTime() < GetTime() - 10 * 60)
            {
                MilliSleep(10000);
                continue;
            }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { while (true) {
        ...
        LogPrintf("Running PyriteMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("PyriteMiner terminated\n");
        throw;
    }
}
