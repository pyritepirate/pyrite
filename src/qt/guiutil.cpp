...
{
    // Convert pyrite:// to pyrite:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pyrite://"))
    {
        uri.replace(0, 12, "pyrite:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Pyrite.lnk";
}
...
{
    return GetAutostartDir() / "pyrite.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Pyrite-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  pyrite-qt [" + tr("command-line options") + "]                     " + "\n";
